...
const cases = [
  {
    task: 'Flask API task',
    expect: ['python-development', 'flask-development'],
  ...
  {
    task: 'Django app task',
    expect: ['python-development', 'django-development'],
    keywords: ['python', 'django'],
    max: 2
  },
  {
    task: 'FastAPI endpoint task',
    expect: ['python-development', 'fastapi-development'],
  ...
  {
    task: 'route library docs, GitHub overview, and web discovery for research',
    expect: ['evidence-router'],
    keywords: ['choosing'],
    max: 1
  },
  ...
  {
    task: 'find legitimate GitHub PR contribution opportunities and qualify the safest issue',
    expect: ['github-contribution-opportunities'],
    keywords: ['contribution'],
    max: 1
  },
  {
    task: 'execute ordered multi-task plan with acceptance review',
    expect: ['task-scoped-execution'],
    keywords: ['ordered'],
    max: 1
  },
  {
    task: 'UX review of an existing app for goal, flow, state, and copy evidence',
    expect: ['ux-review'],
    keywords: ['goal'],
    max: 1
  },
  {
    task: "map an app's journeys and service seams before implementation",
    expect: ['app-experience-mapping'],
    keywords: ['journey'],
    max: 1
  },
  {
    task: "review an app's full experience across journeys and seams before shipping",
    expect: ['app-experience-mapping', 'black-box-experience-review'],
    keywords: ['journey', 'seam'],
    max: 2
  },
  {
    task: 'write an Upwork application proposal for a virtual assistant role',
    expect: ['upwork-proposals'],
    keywords: ['proposal'],
    max: 1
  }
]
...
for (const c of cases) {
  ...
  if (problems.length) {
    failures++
    console.log('FAIL ' + c.task + ': ' + problems.join('; '))
...
for (const id of ['pack-backend', 'pack-toolchains']) {
  ...
  if (/no more than two leaves/i.test(text)) console.log('PASS ' + id + ' documents the two-leaf rule')
    ...
    failures++
    console.log('FAIL ' + id + ' lacks the two-leaf rule')
  }
}
...
if (/dispatch|delegate|subagent|agent/i.test(tse) && !/unsupported|never dispatch|no subagent/i.test(tse)) {
  failures++
  console.log('FAIL task-scoped-execution lacks a no-dispatch rule')
