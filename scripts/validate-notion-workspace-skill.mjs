...
try {
  ...
  /* Missing skill is reported by the checks below. */
}
...
check(
  /notion-cli auth status/.test(skill) && /search/.test(skill) && /fetch/.test(skill),
  'skill verifies auth and discovers existing structure before writes'
)
check(
...
check(
  /Projects/.test(skill) &&
    ...
    /Content/.test(skill) &&
    /Learning/.test(skill),
  'skill supports a flexible second-brain hub'
)
check(
  /Second Brain/.test(skill) && /single central hub|only central dashboard/i.test(skill),
  'skill makes the existing top-level Second Brain the single central hub'
)
check(
  /Creator’s Companion/.test(skill) && /content system/i.test(skill) && /reuse/i.test(skill),
  'skill reuses Creator’s Companion as the content system'
)
